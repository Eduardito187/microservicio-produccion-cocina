...
# Register service with Consul (optional)
if [ "${RUN_CONSUL_REGISTER:-1}" = "1" ]; then
  echo "[entrypoint] registering service with Consul..."
  php artisan consul:register >/proc/1/fd/1 2>/proc/1/fd/2 || echo "[entrypoint] WARN: consul:register failed (continuing)"
  trap 'echo "[entrypoint] deregistering from Consul..."; php artisan consul:deregister >/proc/1/fd/1 2>/proc/1/fd/2 || true' TERM INT
fi
