...
/**
 * Estos tests unitarios NO prueban DB ni HTTP. Su objetivo es:
 * - subir coverage de Application (Commands + Handlers)
 * - verificar el wiring básico de repositorio + transaction wrapper
 *
 * Nota: si quieres escalar este patrón al resto de maestros (Estacion/Etiqueta/etc.)
 * basta con copiar el bloque y cambiar el Entity/Repo/Commands.
 */
class MaestrosHandlersCrudTest extends TestCase
{
    ...
    public function test_calendario_crud_handlers_invocan_repositorio_y_mapean_respuesta(): void
    {
        ...
        $repo->expects($this->once())
            ->method('save')
            ->with($this->callback(function (Calendario $c): bool {
                return $c->id === null
                    && $c->fecha->format('Y-m-d') === '2026-01-10'
                    && $c->sucursalId === 'SCZ-001';
            }))
            ->willReturn(10);

        $crear = new CrearCalendarioHandler($repo, $this->tx());
        ...
        $repo2 = $this->createMock(CalendarioRepositoryInterface::class);
        $repo2->method('byId')->with(10)->willReturn($existing);
        $repo2->expects($this->once())->method('save')->willReturn(10);

        $actualizar = new ActualizarCalendarioHandler($repo2, $this->tx());
        ...
        // Ver
        $repo3 = $this->createMock(CalendarioRepositoryInterface::class);
        $repo3->method('byId')->with(10)->willReturn($existing);
        $ver = new VerCalendarioHandler($repo3, $this->tx());
        $data = $ver(new VerCalendario(10));
        $this->assertSame(['id' => 10, 'fecha' => '2026-01-11', 'sucursal_id' => 'SCZ-002'], $data);

        // Listar
        $repo4 = $this->createMock(CalendarioRepositoryInterface::class);
        $repo4->method('list')->willReturn([$existing]);
        $listar = new ListarCalendariosHandler($repo4, $this->tx());
        $list = $listar(new ListarCalendarios());
        $this->assertCount(1, $list);
        $this->assertSame(10, $list[0]['id']);

        // Eliminar
        $repo5 = $this->createMock(CalendarioRepositoryInterface::class);
        $repo5->method('byId')->with(10)->willReturn($existing);
        $repo5->expects($this->once())->method('delete')->with(10);

        $eliminar = new EliminarCalendarioHandler($repo5, $this->tx());
    ...
    public function test_direccion_crud_handlers_invocan_repositorio_y_mapean_respuesta(): void
    {
        ...
        $repo->expects($this->once())
            ->method('save')
            ->with($this->callback(function (Direccion $d): bool {
                return $d->id === null
                    && $d->nombre === 'Casa'
                    && $d->linea1 === 'Av. Siempre Viva 123'
                    && $d->ciudad === 'SCZ'
                    && $d->geo === ['lat' => -17.78, 'lng' => -63.18];
            }))
            ->willReturn(20);

        $crear = new CrearDireccionHandler($repo, $this->tx());
        $id = $crear(new CrearDireccion(
            ...
            provincia: null,
            pais: 'BO',
            geo: ['lat' => -17.78, 'lng' => -63.18]
        ));
        $this->assertSame(20, $id);

        // Actualizar
        $existing = new Direccion(20, 'Casa', 'Av. Siempre Viva 123', null, 'SCZ', null, 'BO', ['lat' => -17.78, 'lng' => -63.18]);
        $repo2 = $this->createMock(DireccionRepositoryInterface::class);
        $repo2->method('byId')->with(20)->willReturn($existing);
        $repo2->expects($this->once())->method('save')->willReturn(20);

        $actualizar = new ActualizarDireccionHandler($repo2, $this->tx());
        ...
        // Ver
        $repo3 = $this->createMock(DireccionRepositoryInterface::class);
        $repo3->method('byId')->with(20)->willReturn($existing);
        $ver = new VerDireccionHandler($repo3, $this->tx());
        $data = $ver(new VerDireccion(20));
        $this->assertSame(20, $data['id']);
        $this->assertSame('Calle 1', $data['linea1']);

        // Listar
        $repo4 = $this->createMock(DireccionRepositoryInterface::class);
        $repo4->method('list')->willReturn([$existing]);
        $listar = new ListarDireccionesHandler($repo4, $this->tx());
        $list = $listar(new ListarDirecciones());
        $this->assertCount(1, $list);
        $this->assertSame('Oficina', $list[0]['nombre']);

        // Eliminar
        $repo5 = $this->createMock(DireccionRepositoryInterface::class);
        $repo5->method('byId')->with(20)->willReturn($existing);
        $repo5->expects($this->once())->method('delete')->with(20);

        $eliminar = new EliminarDireccionHandler($repo5, $this->tx());
     ...
     */
    public function test_crear_handler_llama_save_y_devuelve_id(array $cfg): void
    {
        $repo = $this->createMock($cfg['repo']);
        $repo->expects($this->once())
            ->method('save')
            ->with($this->isInstanceOf($cfg['entity']))
            ->willReturn(123);

        $handler = new ($cfg['handlers']['crear'])($repo, $this->tx());
        $id = $handler($cfg['commands']['crear']());
     ...
     * @dataProvider maestrosProvider
     */
    public function test_actualizar_handler_hace_byId_modifica_y_save(array $cfg): void
    {
        $repo = $this->createMock($cfg['repo']);

        $entity = $cfg['makeEntity'](); // entidad existente
        $repo->expects($this->once())
            ->method('byId')
            ->with(10)
            ->willReturn($entity);
        ...
        $repo->expects($this->once())
            ->method('save')
            ->with($this->callback(function ($saved) use ($cfg): bool {
                if (!($saved instanceof $cfg['entity'])) return false;

                foreach ($cfg['expectedAfterUpdate']() as $prop => $expected) {
        ...
        $handler = new ($cfg['handlers']['actualizar'])($repo, $this->tx());
        $id = $handler($cfg['commands']['actualizar']());
     ...
     * @dataProvider maestrosProvider
     */
    public function test_eliminar_handler_hace_byId_y_delete(array $cfg): void
    {
        $repo = $this->createMock($cfg['repo']);
        ...
        $repo->expects($this->once())
            ->method('byId')
            ->with(10)
            ->willReturn($cfg['makeEntity']());

        $repo->expects($this->once())
            ->method('delete')
            ->with(10);

        $handler = new ($cfg['handlers']['eliminar'])($repo, $this->tx());
        $handler($cfg['commands']['eliminar']());
     ...
     * @dataProvider maestrosProvider
     */
    public function test_ver_handler_mapea_correcto(array $cfg): void
    {
        $repo = $this->createMock($cfg['repo']);
        ...
        $handler = new ($cfg['handlers']['ver'])($repo, $this->tx());
        $out = $handler($cfg['commands']['ver']());

        $this->assertSame($cfg['expectedView'](), $out);
    }

    /**
     * @dataProvider maestrosProvider
     */
    public function test_listar_handler_mapea_lista_correcto(array $cfg): void
    {
        $repo = $this->createMock($cfg['repo']);
        ...
        $handler = new ($cfg['handlers']['listar'])($repo, $this->tx());
        $out = $handler($cfg['commands']['listar']());
    ...
    public static function maestrosProvider(): array
    {
        ...
        return [
            // --- TU PROVIDER COMPLETO QUEDA IGUAL ---
            // (No lo toqué: solo faltaba el dataProvider del test_crear)
            'CalendarioItem' => [[
            ...
            'VentanaEntrega' => [[
                ...
                'expectedView' => fn() => [
                    'id' => 10,
                    'desde' => $dtDesde->format('Y-m-d H:i:s'),
                    'hasta' => $dtHasta->format('Y-m-d H:i:s'),
                ],
            ]],
        ];
    }
