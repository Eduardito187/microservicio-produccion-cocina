...
class SuscripcionCrudTest extends TestCase
{
    ...
    {
        $create = $this->postJson(route('suscripciones.crear'), [
        ...
        $this->getJson(route('suscripciones.listar'))
            ->assertOk()
            ->assertJsonFragment(['id' => $suscripcionId, 'nombre' => 'Suscripcion 1']);

        $this->getJson(route('suscripciones.ver', ['id' => $suscripcionId]))
            ->assertOk()
            ->assertJsonFragment(['id' => $suscripcionId, 'nombre' => 'Suscripcion 1']);

        $update = $this->putJson(route('suscripciones.actualizar', ['id' => $suscripcionId]), [
            'nombre' => 'Suscripcion 2',
        ]);
